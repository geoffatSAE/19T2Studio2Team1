...
Shader "Custom/JellyShader" {
	...
	SubShader {
		...
		#pragma surface surf Standard nolighting noshadows addshadow vertex:vert
		...
		void vert (inout appdata_base v) {
			...
			float4 origin = _ImpactOrigin + (_Time.x * 25.f) * _ImpactSpeed * direction;
			...
			v.vertex.xyz += v.normal * sin(impactAxis * _Frequency + (_Time.x * 25.f) * _WaveSpeed) * _Amplitude * (1 / dist);
		}
